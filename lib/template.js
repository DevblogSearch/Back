module.exports = {
  ...
  parseSearchResponse(searchResponse, q, start, numFound, user) {
    let searchPage = `
      ...
      <div class="col-xs-12 col-sm-12 col-md-7 col-md-offset-1 col-lg-6 col-lg-offset-1">
    `
    ...
    for (res of searchResponse) {
      ...
      if (!!user) {
        likeButton = `<button class = "button-like visible-lg-block visible-md-block" onclick="likeEvent(${user.id}, '${res.url}')"><i class="fa fa-heart"></i><span>like</span></button><button class = "mobile-heart visible-xs-block visible-sm-block" onclick="likeEvent(${user.id}, '${res.url}')"><i class="fa fa-heart"></i><span></span></button>`;
      }
