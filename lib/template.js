module.exports = {
  ...
  parseSearchResponse(searchResponse, q) {
    let searchPage = `
      <div class="row">
      <div class="col-md-5">
    `
    ...
    for (res of searchResponse) {
      ...
      <li class="search-item clearfix">
        <div class="search-content text-left">
          <a>
            <form method="POST" action="http://127.0.0.1:3000/ping_events">
              <input type="hidden" name="url" value="${res.url}">
              <input type="hidden" name="blog_id" value="${res.blog_id}">
              <h3>
                <input type="submit" class="search-result-title" value="${res.title}">
              </h3>
            </form>
          </a>
          <h4>
    ...
    `;
    for (let i = 1; i <= 5; i++) {
      searchPagination += `<li class="page-item"><a class="page-link" href="/search?q=${q}&start=${i}&n=10">${i}</a></li>`;
    }
    searchPagination += `
      </ul>
      </div>
